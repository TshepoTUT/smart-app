...
// app/data/Organiser/myEvents.js

// ... (keep all your existing imports and functions)

// ✅ NEW: Function to fetch public/approved events for the landing page
export const getApprovedEventsPublic = async () => {
  try {
    // Use the public endpoint (GET /events) which usually returns published events
    const response = await fetch(`${API_URL}/events?page=1&pageSize=50`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      // If the public endpoint fails, just return empty array to prevent crash
      console.warn(`Public events fetch failed: ${response.status}`);
      return [];
    }
    ...
    // Map backend data to the format your landing page expects
    return events.map((event) => ({
      title: event.name || event.title,
      id: event.id || event.uuid,
      // Map dates using your existing helpers
      displayDate: formatDisplayDate(event.startDateTime),
      time: formatTimeRange(event.startDateTime, event.endDateTime),
      description: event.description,
      location: event.venue?.name || event.venue?.location || 'Venue Name Missing',
      category: event.Theme?.name || 'General', // Assuming Theme has a name
